...
- **個人化記憶（選用，預設關閉）**：可在設定頁啟用，讓 AI 漸進學習你的命名與指令風格；啟用後會將過往的群組名稱與指令送回 AI 供應商做摘要，產生少量額外用量，樣本與摘要僅存本機（詳見 PRIVACY）
...
- **預設關閉**。要啟用請到設定頁勾選「個人化記憶」。
- 啟用後，每次成功分組時會在本機紀錄三類字串：你親自重新命名過的群組名稱、AI 命名後你沿用的群組名稱、你下過的指令。每類都只保留近期固定數量。
- 累積足夠樣本後，會額外呼叫一次你選定的 AI 供應商把樣本摘要成一段提示，下次分組時注入給 AI 參考——這會在你的供應商帳戶**產生少量額外用量**。
- 樣本與摘要僅存於 `chrome.storage.local`（不同步、不上傳）；關掉開關會立刻停止記錄與摘要呼叫，本機既有資料保留。
- 細節見 [PRIVACY.md](./PRIVACY.md) 的「選用功能：個人化記憶」段落。
...
- 「個人化記憶」**預設關閉**；啟用後才會額外把你過往的群組名稱與指令送回 AI 供應商做摘要，樣本與摘要僅存本機
